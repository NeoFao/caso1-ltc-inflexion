...
alterarse sin pérdida, porque cada observación es independiente por diseño; en una serie temporal
la observación en `t` está relacionada con la de `t-1`, y esa dependencia es lo que hay que
modelar. Reordenarla no la desordena, la destruye.
...
sino una decisión de muestreo —velas diarias— sobre un activo que cotiza de forma continua las
veinticuatro horas. Todo el análisis posterior presupone que ese orden es información.
...
calendario: en la bolsa tradicional está documentado desde hace décadas un efecto de fin de semana,
con rendimientos distintos según el día en que cierra la operación (French, 1980). El **ciclo** es
un movimiento de duración variable, asociado a factores macroeconómicos. El **residuo** es lo que
queda al retirar los tres anteriores.

Sobre LTC la estacionalidad semanal es marginal, y la explicación es directa: el mercado de
...
French (1980). La consecuencia para el modelo es que codificar el día de la semana como variable no
aportaría señal.
...
descripción visual: una serie puede parecer estable en un gráfico sin serlo, y por eso se prueba de
forma formal.

El **test de Dickey-Fuller aumentado (ADF)** es el instrumento estándar para esa prueba (Dickey &
...
procedimiento a las seis series, en nivel y en retornos: `adfuller` con selección automática de
rezagos por criterio de información de Akaike.

Conviene precisar qué permite afirmar el resultado. Que el test no rechace la nula sobre los
precios **no demuestra** que sean no estacionarios: demuestra que no hay evidencia suficiente en
contra. Un test de hipótesis nunca prueba la nula, solo puede fallar en rechazarla. Sobre retornos,
en cambio, el rechazo con p < 0,000001 sí constituye evidencia fuerte a favor de la
estacionariedad.

**ETH** merece mención aparte: su p-valor en nivel, 0,059, queda apenas por encima del umbral. No
cambia la conclusión, pero recuerda que el resultado de un test depende de la ventana muestral, y
que con otro período ese caso podría cruzar el umbral en cualquiera de los dos sentidos.

---

## 4. No estacionariedad

El mismo resultado, leído en sentido inverso, indica que los precios de las seis criptomonedas son
compatibles con un proceso de tendencia estocástica —un paseo aleatorio con deriva en el caso más
simple— y no con uno que oscile alrededor de una media fija (Nelson & Plosser, 1982). Dos síntomas
lo delatan sin necesidad del test: la media no es estable en ninguna ventana razonable, porque LTC
recorre de **40,92** a **387,80** dentro de la propia muestra; y la varianza tampoco, porque un
movimiento porcentual constante produce un movimiento absoluto proporcional al nivel.
...
`r_t = (p_t − p_{t-1}) / p_{t-1}`— resuelve el problema porque cambia la pregunta: de "cuánto vale
el precio", que acumula nivel desde el origen de la serie, a "cuánto cambió en esta vela", que no.
La sección 3 confirma que funciona sobre estos datos. Por eso, y no por costumbre de la disciplina,
**las características del modelo se construyen sobre retornos y no sobre precios en nivel**.
...
Un proceso es **homocedástico** cuando la dispersión de sus incrementos es constante en el tiempo,
y **heterocedástico** cuando esa dispersión cambia, concentrándose en episodios o regímenes (Engle,
1982). Engle formalizó la idea para series financieras con el modelo ARCH, mostrando que la
varianza condicional puede depender del historial reciente aunque la media no muestre ningún patrón
predecible: puede haber estructura en la varianza sin haberla en el nivel.
...
tramos alternos. El cociente medido es **1,28** sin regímenes —cercano a 1, con la desviación
propia de estimar sobre muestra finita— y **6,41** con ellos, capturando el factor introducido. El
procedimiento detecta la heterocedasticidad cuando existe y no la inventa cuando no existe, que es
la comprobación necesaria antes de aplicarlo a LTC.
...
subestiman el riesgo en los períodos agitados. La literatura sobre criptoactivos documenta este
patrón de conmutación entre regímenes, con modelos GARCH de cambio markoviano ajustando mejor que
uno de régimen único (Caporale & Zekokh, 2019).
...
dos series de idéntica construcción y semilla, variando un solo parámetro: la desviación estándar
de los retornos. Los dos niveles no son arbitrarios, sino los extremos de la volatilidad móvil que
se mide enseguida sobre Litecoin, de manera que el rango construido cubre el que se observa en el
...
Aplicado a la serie real, el resultado es que la volatilidad de LTC no es constante. El cociente de
**8,8** entre extremos es muy superior al 1,28 de la serie construida sin regímenes y del mismo
orden que el 6,41 de la construida con ellos: LTC no se comporta como una serie de volatilidad
estable sino como una que conmuta entre regímenes de intensidad muy distinta, consistente con lo
que reporta la literatura GARCH sobre criptoactivos (Katsiampa, 2017).

En la Figura 6 los picos de volatilidad coinciden con los tramos de movimiento más pronunciado en
cualquiera de las dos direcciones: la volatilidad mide la magnitud del cambio, no su signo. Este
resultado es, junto con el de la sección 3, la segunda evidencia medida sobre datos reales de que
los métodos que asumen varianza constante no sirven para este problema.
...
información lineal sobre su futuro, que en este proyecto equivale a preguntar si tiene sentido usar
precios rezagados como variable de entrada.

**En nivel** la autocorrelación decae muy lentamente al aumentar el rezago: es la firma de una
serie no estacionaria, donde cada observación es casi idéntica a la anterior porque comparten la
mayor parte de su nivel acumulado. Confirma con otra herramienta lo que ya mostró el test ADF.

**En retornos**, de los 40 rezagos evaluados solo tres —el 8, el 14 y el 31— salen de la banda de
confianza al 95 %.
...
rezagados de LTC dispondría de muy poca estructura que explotar. No es un resultado negativo: **es
la justificación del enfoque**. Es consistente con la hipótesis de mercados eficientes en su forma
débil —los precios pasados no permiten anticipar rendimientos futuros mediante reglas lineales
simples (Fama, 1970)— y es, medido sobre datos propios, el argumento a favor de recurrir a modelos
no lineales y multivariantes. Si tres rezagos de cuarenta bastaran para predecir, un modelo lineal
...
autocorrelación de la sección 7. Es el instrumento que permite responder si el planteamiento
multivariante del enunciado tiene sustento: si LTC no se moviera de forma relacionada con las otras
cinco criptomonedas, incorporarlas no aportaría información.

La Figura 8 pone tres matrices 6×6 lado a lado. Las dos primeras corresponden a paneles construidos
por nosotros con correlación fijada de antemano; la tercera, a los retornos reales. Puestas juntas,
permiten leer el resultado real contra dos referencias donde la respuesta la pusimos nosotros.

La distorsión que introducen los precios en nivel no consiste en que la correlación resulte alta,
sino en que resulta **errática**. Sobre nivel, la pareja LTC–BTC —el activo que marca la tendencia
de todo el sector— cae a 0,126, mientras LTC–ADA sube a 0,796: un ordenamiento económicamente
implausible. Sobre retornos el orden se corrige y el rango se estrecha a la mitad.

La explicación es que la correlación en nivel mide algo distinto de lo que se busca. Dos series con
tendencia comparten tendencia, y la correlación entre ellas captura esa coincidencia de dirección
de largo plazo más que la codependencia real de sus movimientos período a período. Es el fenómeno
de la **correlación espuria** descrito por Granger y Newbold (1974): dado que la sección 3 midió que
los precios no son estacionarios, cualquier correlación calculada sobre ellos hereda ese defecto.
Por eso la correlación cruzada del proyecto se calcula sobre retornos.

Sobre retornos, el activo de apoyo más correlacionado con LTC es ETH y el menos, SOL. Ambos valores
quedan lejos de los extremos —ni cerca de 0, que dejaría a las variables de apoyo sin aportar, ni
cerca de 1, que las volvería redundantes—, y esa es la condición que sostiene el planteamiento
multivariante del enunciado.
